...
namespace ActiveRagdoll
{
    ...
    public class PhysicsModule : Module
    {
        ...
        [Header("--- GENERAL ---")]
        [SerializeField]
        private BALANCE_MODE _balanceMode = BALANCE_MODE.STABILIZER_JOINT;
        ...
        [Tooltip(
            "Defines how much torque percent is applied given the inclination angle percent [0, 1]"
        )]
        ...
        [SerializeField]
        private JointDriveConfig _stabilizerJointDrive;
        public JointDriveConfig StabilizerJointDrive
        {
            ...
            set
            {
                if (_stabilizerJoint != null)
                    _stabilizerJoint.angularXDrive = _stabilizerJoint.angularXDrive =
                        (JointDrive)value;
            }
        }
        ...
        [SerializeField]
        private float freezeRotationSpeed = 5;
        ...
        {
            _stabilizerGameobject = new GameObject(
                "Stabilizer",
                typeof(Rigidbody),
        ...
        private void FixedUpdate()
        {
            ...
            switch (_balanceMode)
            {
                case BALANCE_MODE.UPRIGHT_TORQUE:
                    var balancePercent =
                        Vector3.Angle(_activeRagdoll.PhysicalTorso.transform.up, Vector3.up) / 180;
                    balancePercent = uprightTorqueFunction.Evaluate(balancePercent);
                    var rot = Quaternion
                        .FromToRotation(_activeRagdoll.PhysicalTorso.transform.up, Vector3.up)
                        .normalized;

                    _activeRagdoll.PhysicalTorso.AddTorque(
                        new Vector3(rot.x, rot.y, rot.z) * uprightTorque * balancePercent
                    );

                    var directionAnglePercent =
                        Vector3.SignedAngle(
                            _activeRagdoll.PhysicalTorso.transform.forward,
                            TargetDirection,
                            Vector3.up
                        ) / 180;
                    _activeRagdoll.PhysicalTorso.AddRelativeTorque(
                        0,
                        directionAnglePercent * rotationTorque,
                        0
                    );
                    break;

                case BALANCE_MODE.FREEZE_ROTATIONS:
                    var smoothedRot = Quaternion.Lerp(
                        _activeRagdoll.PhysicalTorso.rotation,
                        _targetRotation,
...
#if UNITY_EDITOR
                Debug.LogWarning(
                    "SetBalanceMode was called but the mode selected was the same as "
