...
namespace ActiveRagdoll
{
    public class CameraModule : Module
    {
        ...
        [Tooltip("아래를 내려다 볼 때 카메라가 머리 위로 크레인을 움직이게 합니다," + " 아래로 갈수록 가시성 증가")]
        public bool improveSteepInclinations = true;
        public float inclinationAngle = 30,
            inclinationDistance = 1.5f;

        [Header("--- DISTANCES ---")]
        public float minDistance = 2;
        public float maxDistance = 5,
            initialDistance = 3.5f;
        ...
        {
            _cameraRotation.x = Mathf.Repeat(
                _cameraRotation.x + _inputDelta.x * (invertX ? -1 : 1) * lookSensitivity,
                360
            );
            _cameraRotation.y = Mathf.Clamp(
                _cameraRotation.y + _inputDelta.y * (invertY ? 1 : -1) * lookSensitivity,
                minVerticalAngle,
                maxVerticalAngle
            );
        }
        ...
        private void UpdateCameraPosRot()
        {
            ...
            {
                float anglePercent =
                    (_cameraRotation.y - minVerticalAngle) / (maxVerticalAngle - minVerticalAngle);
                float currentDistance = (
                    (anglePercent * inclinationDistance) - inclinationDistance / 2
                );
                movedLookPoint +=
                    (
                        Quaternion.Euler(inclinationAngle, 0, 0)
                        * Auxiliary.GetFloorProjection(Camera.transform.forward)
                    ) * currentDistance;
            }

            // Smooth
            _smoothedLookPoint = Vector3.Lerp(
                _smoothedLookPoint,
                movedLookPoint,
                smooth ? smoothSpeed * Time.deltaTime : 1
            );
        ...
        private void AvoidObstacles()
        {
            Ray cameraRay = new Ray(
                _lookPoint.position,
                Camera.transform.position - _lookPoint.position
            );
            bool hit = Physics.Raycast(
                cameraRay,
                out RaycastHit hitInfo,
                Vector3.Distance(Camera.transform.position, _lookPoint.position),
                ~dontBlockCamera
            );
            ...
            if (hit)
            {
                Camera.transform.position =
                    hitInfo.point + (hitInfo.normal * cameraRepositionOffset);
                Camera.transform.LookAt(_smoothedLookPoint);
            }
        }
        ...
        public void OnScrollWheel(InputValue value)
        {
            ...
            _currentDistance = Mathf.Clamp(
                _currentDistance + scrollValue.y / 1200 * -scrollSensitivity,
                minDistance,
                maxDistance
            );
        }
