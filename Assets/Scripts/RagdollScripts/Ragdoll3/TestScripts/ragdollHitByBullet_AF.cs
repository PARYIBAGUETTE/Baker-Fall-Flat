...
namespace AnimFollow
{
    public class ragdollHitByBullet_AF : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            {
                Debug.LogWarning(
                    "You need to assign blood prefab in the ragdollHitByBullet script on "
                        + this.name
                );
            ...
            {
                Debug.LogWarning(
                    "The ragdollHitByBullet script on "
                        + this.name
                        + " requires a RagdollControl script to work"
                );
                userNeedsToFixStuff = true;
            }
        }
        ...
        void HitByBullet(BulletHitInfo_AF bulletHitInfo)
        {
            ...
            bloodClone =
                Instantiate(
                    blood,
                    bulletHitInfo.hitPoint,
                    Quaternion.LookRotation(bulletHitInfo.hitNormal)
                ) as ParticleSystem;
        ...
        IEnumerator AddForceToLimb(BulletHitInfo_AF bulletHitInfo)
        {
            ...
            bulletHitInfo.hitTransform
                .GetComponent<Rigidbody>()
                .AddForceAtPosition(bulletHitInfo.bulletForce, bulletHitInfo.hitPoint);
        }
    }
}
