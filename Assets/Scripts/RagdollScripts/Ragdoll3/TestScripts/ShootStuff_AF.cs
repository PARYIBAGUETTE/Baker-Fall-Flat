...
namespace AnimFollow
{
    ...
    public class ShootStuff_AF : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            {
                Debug.LogWarning(
                    "You need to assign a camera to the ShootStuff script on " + this.name
                );
            ...
            {
                Debug.LogWarning(
                    "You need to assign crosshairTexture in the ShootStuff script on " + this.name
                );
                userNeedsToFixStuff = true;
            }
            else
        ...
        void Update()
        {
            ...
            )
            {
                ...
                Ray rayen = theCamera.ScreenPointToRay(
                ...
                if (Physics.Raycast(rayen, out raycastHit, 100f))
                {
                    ...
                    bulletHitInfo.bulletForce =
                        (raycastHit.point - transform.position).normalized * bulletForce;
                    bulletHitInfo.hitNormal = raycastHit.normal;
                    bulletHitInfo.hitPoint = raycastHit.point;

                    raycastHit.transform.root.SendMessage(
                        "HitByBullet",
                        bulletHitInfo,
        ...
        void OnGUI()
        {
            ...
            bulletForce = GUI.HorizontalSlider(
                new Rect(10, 105, 150, 15),
                bulletForce,
                1000f,
                20000f
            );
