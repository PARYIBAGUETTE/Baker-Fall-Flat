...
namespace AnimFollow
{
    public class CameraMovement_AF : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            relCameraPos = new Vector3(
                Vector3.Dot(transform.position - lookAtTransform.position, lookAtTransform.forward),
                transform.position.y - lookAtTransform.position.y,
                Vector3.Dot(transform.position - lookAtTransform.position, lookAtTransform.up)
            );
        }
        ...
        void FixedUpdate() // The camera is not smooth unless in FixedUpdate
        {
            ...
            Vector3 absCameraPos =
                ...
                + relCameraPos.x
                    * new Vector3(
                        lookAtTransform.forward.x,
                        0f,
                        lookAtTransform.forward.z
                    ).normalized
                + relCameraPos.y * Vector3.up
                + relCameraPos.z
                    * new Vector3(lookAtTransform.up.x, 0f, lookAtTransform.up.z).normalized;
