...
namespace AnimFollow
{
    public partial class SimpleFootIK_AF
    {
        void Awake2()
        {
            ...
            {
                Debug.LogWarning(
                    "ragdoll not assigned in SimpleFootIK script on "
                        + this.name
            ...
            else
            {
                ...
                if (!ragdollOnIgnoredLayer)
                {
                    Debug.LogWarning(
                        "Layer for "
                            + ragdoll.name
                            + " and its children must be set to an ignored layer"
                            + "\n"
                    );
                    userNeedsToFixStuff = true;
                }
            ...
            for (int n = 0; n < characterTransforms.Length; n++)
            {
                if (
                    (
                        characterTransforms[n].name.ToLower().Contains("foot")
                        && characterTransforms[n].name.ToLower().Contains("left")
                    )
                )
                {
                ...
                if (
                    characterTransforms[n].name.ToLower().Contains("foot")
                    && characterTransforms[n].name.ToLower().Contains("right")
                )
                {
