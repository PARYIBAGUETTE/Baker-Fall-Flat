...
namespace AnimFollow
{
    public class AnimFollow_AF : MonoBehaviour
    {
        ...
        [Range(0f, 100f)]
        public float maxTorque = 100f; // Limits the world space torque

        [Range(0f, 100f)]
        public float maxForce = 100f; // Limits the force

        [Range(0f, 10000f)]
        public float maxJointTorque = 10000f; // Limits the force

        [Range(0f, 10f)]
        public float jointDamping = .6f; // Limits the force

        public float[] maxTorqueProfile =
        {
            100f,
            100f,
            100f,
        ...
        [Range(0f, .64f)]
        public float PTorque = .16f; // For all limbs Torque strength

        [Range(0f, 160f)]
        public float PForce = 30f;

        [Range(0f, .008f)]
        public float DTorque = .002f; // Derivative multiplier to PD controller

        [Range(0f, .064f)]
        public float DForce = .01f;

        //	public float[] PTorqueProfile = {20f, 30f, 10f, 30f, 10f, 30f, 30f, 30f, 10f, 30f, 10f}; // Per limb world space torque strength
        public float[] PTorqueProfile =
        {
            20f,
            30f,
            10f,
            ...
            30f,
            10f
        }; // Per limb world space torque strength for EthanRagdoll_12 (twelve rigidbodies)
        public float[] PForceProfile = { 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f };

        // The ranges are not set in stone. Feel free to extend the ranges
        [Range(0f, 340f)]
        public float angularDrag = 100f; // Rigidbodies angular drag. Unitys parameter
        ...
        [SerializeField]
        bool torque = false; // Use World torque to controll the ragdoll (if true)

        [SerializeField]
        bool force = true; // Use force to controll the ragdoll

        [HideInInspector]
        public bool mimicNonRigids = true; // Set all local rotations of the transforms without rigidbodies to match the local rotations of the master
        ...
        [HideInInspector]
        public Vector3 totalTorqueError; // Total world space angular error of all limbs. This is a vector.
        ...
        [HideInInspector]
        public Vector3 totalForceError; // Total world position error. a vector.
        ...
        void Awake() // Initialize
        {
            ...
            if (!master)
            {
                UnityEngine.Debug.LogWarning(
                    "master not assigned in AnimFollow script on " + this.name + "\n"
                );
            ...
            {
                UnityEngine.Debug.LogWarning(
                    "Missing script SimpleFootIK_AF on "
                        + master.name
            ...
            {
                UnityEngine.Debug.LogWarning(
                    "Missing script RagdollControl on "
                        + this.name
                        + ".\nAdd it or comment out the directive from top line in the AnimFollow script."
                        + "\n"
                );
                ...
                if (slaveTransform.GetComponent<Rigidbody>())
                {
                    ...
                    if (slaveTransform.GetComponent<ConfigurableJoint>())
                    {
                        ...
                        Vector3 forward = Vector3.Cross(
                            configurableJoints[j].axis,
                            configurableJoints[j].secondaryAxis
                        );
                    ...
                    else if (j > 0)
                    {
                        UnityEngine.Debug.LogWarning(
                            "Rigidbody "
                                ...
                                + " on "
                                + this.name
                                + " is not connected to a configurable joint"
                                + "\n"
                        );
                        userNeedsToAssignStuff = true;
                        return;
                    }
                    rigidbodiesPosToCOM[j] =
                        Quaternion.Inverse(slaveTransform.rotation)
                        * (
                            slaveTransform.GetComponent<Rigidbody>().worldCenterOfMass
                            - slaveTransform.position
                        );
                    j++;
                }
                else
            ...
            {
                UnityEngine.Debug.LogWarning(
                    "There are no configurable joints on the ragdoll "
                        + this.name
            ...
            if (slaveRigidTransforms.Length == 0)
                UnityEngine.Debug.LogWarning(
                    "There are no rigid body components on the ragdoll " + this.name + "\n"
                );
            ...
            if (PTorqueProfile[PTorqueProfile.Length - 1] == 0f)
                UnityEngine.Debug.Log(
                    "The last entry in the PTorqueProfile is zero on "
                        + this.name
            ...
            {
                UnityEngine.Debug.Log(
                    "Should you not assign some slaveExcludeTransforms to the AnimFollow script on "
                        + this.name
                        + "\n"
                );
            }
        ...
        void Start()
        {
            ...
            foreach (Transform slaveRigidTransform in slaveRigidTransforms) // Set some of the Unity parameters
            {
                ...
                slaveRigidTransform.GetComponent<Rigidbody>().maxAngularVelocity =
                    maxAngularVelocity;
                i++;
            }
        }
        ...
        public void DoAnimFollow()
        {
            ...
            if (frameCounter % 2 == 0)
            {
                for (int i = 2; i < slaveTransforms.Length - 1; i++) // Set all local rotations of the transforms without rigidbodies to match the local rotations of the master
                {
                    ...
                    {
                        localRotations1[i] = Quaternion.Lerp(
                            localRotations1[i],
                            localRotations2[i],
                            2f / secondaryUpdate
                        );
            ...
            for (int i = 0; i < slaveRigidTransforms.Length; i++) // Do for all rigid bodies
            {
                ...
                if (torque) // Calculate and apply world torque
                {
                    targetRotation =
                        ...
                        * Quaternion.Inverse(slaveRigidTransforms[i].rotation);
                    ...
                    if (torqueAngle != 360f)
                    {
                        ...
                        PDControl(
                            PTorque * PTorqueProfile[i],
                            DTorque,
                            out torqueSignal,
                            torqueError,
                            ref torqueLastError[i],
                            reciFixedDeltaTime
                        );
                    }
                    else
                    ...
                    torqueSignal = Vector3.ClampMagnitude(
                        torqueSignal,
                        maxTorque * maxTorqueProfile[i]
                    );
                    slaveRigidbodies[i].AddTorque(torqueSignal, ForceMode.VelocityChange); // Add torque to the limbs
                }

                // Force error
                Vector3 masterRigidTransformsWCOM =
                    masterRigidTransforms[i].position
                    + masterRigidTransforms[i].rotation * rigidbodiesPosToCOM[i];
                forceError =
                    masterRigidTransformsWCOM
                    - slaveRigidTransforms[i].GetComponent<Rigidbody>().worldCenterOfMass; // Doesn't work if collider is trigger
                totalForceError += forceError * forceErrorWeightProfile[i];

                if (force) // Calculate and apply world force
                {
                    PDControl(
                        PForce * PForceProfile[i],
                        DForce,
                        out forceSignal,
                        forceError,
                        ref forceLastError[i],
                        reciFixedDeltaTime
                    );
                    forceSignal = Vector3.ClampMagnitude(
                        forceSignal,
                        maxForce * maxForceProfile[i]
                    );
                    slaveRigidbodies[i].AddForce(forceSignal, ForceMode.VelocityChange);
                }

                if (i > 0)
                    configurableJoints[i].targetRotation =
                        Quaternion.Inverse(localToJointSpace[i])
