    ...
    public class ResizeProfiles_AF : MonoBehaviour
    {
        ...
        void Start()
        {
            if (animFollow_S = GetComponent<AnimFollow_AF>())
            {
                if (animFollow_S.version != version)
                    Debug.LogWarning(
                        "AnimFollow on "
                            ...
                            + " is not version "
                            + version
                            + " but the ResizeProfiles script is"
                    );
                ...
                for (int n = 1; n <= j - i; n++)
                    animFollow_S.maxJointTorqueProfile[j - n] = animFollow_S.maxJointTorqueProfile[
                        i - 1
                    ];
                ...
                for (int n = 1; n <= j - i; n++)
                    animFollow_S.jointDampingProfile[j - n] = animFollow_S.jointDampingProfile[
                        i - 1
                    ];
                ...
                for (int n = 1; n <= j - i; n++)
                    animFollow_S.forceErrorWeightProfile[j - n] =
                        animFollow_S.forceErrorWeightProfile[i - 1];
            }
            else
            {
                Debug.LogWarning(
                    "There is no AnimFollow script on this game object. \nUnable to resize profiles"
                );
            }
