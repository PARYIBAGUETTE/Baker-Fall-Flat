...
namespace AnimFollow
{
    public partial class SimpleFootIK_AF
    {
        ...
        [Range(4f, 20f)]
        public float raycastLength = 5f; // Character must not be higher above ground than this.
        ...
        [Range(1f, 100f)]
        public float footNormalLerp = 40f; // Lerp smoothing of foot normals

        [Range(1f, 100f)]
        public float footTargetLerp = 40f; // Lerp smoothing of foot position

        [Range(1f, 100f)]
        public float transformYLerp = 20f; // Lerp smoothing of transform following terrain

        [HideInInspector]
        public float extraYLerp = 1f; // Used by ragdollControl

        [Range(0f, 1f)]
        public float maxIncline = .8f; // Foot IK not aktiv on inclines steeper than arccos(maxIncline);

        public bool followTerrain = true;

        [HideInInspector]
        public bool userNeedsToFixStuff = false;
